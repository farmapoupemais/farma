-- Schema da Farmácia Poupe Mais para Supabase (PostgreSQL)
...
-- 7. Tabela de Reservas de Estoque por Pedido
...
-- Trigger Automático: O primeiro usuário registrado no Supabase Auth vira OWNER automaticamente!
CREATE OR REPLACE FUNCTION public.handle_new_user_role()
RETURNS trigger AS $$
BEGIN
  IF (SELECT COUNT(*) FROM public.user_roles) = 0 THEN
    INSERT INTO public.user_roles (email, role, created_by)
    VALUES (NEW.email, 'owner', 'system')
    ON CONFLICT (email) DO UPDATE SET role = 'owner';
  ELSE
    INSERT INTO public.user_roles (email, role, created_by)
    VALUES (NEW.email, 'customer', 'system')
...
-- Carga dos 48 Produtos Oficiais da Farmácia Poupe Mais
