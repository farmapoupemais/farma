...
export function CartView() {
  ...
  async function placeOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ items: lines.map(({ id, quantity }) => ({ id, quantity })), fulfillment, coupon: couponApplied ? coupon : "", address: fulfillment === "delivery" ? { cep: data.get("cep"), street: data.get("street"), number: data.get("number"), complement: data.get("complement") } : null }) });
