...
export function RoleDashboard({ initialRole, userName, userEmail, demo = false }: { initialRole: DashboardRole; userName: string; userEmail: string; demo?: boolean }) {
  ...
  async function handleDeleteProduct(id: string) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? "Não foi possível remover o produto.");
  ...
  async function submitDemoOrApi(event: React.FormEvent<HTMLFormElement>, endpoint: string) {
    ...
    if (demo) {
      if (endpoint === "/api/admin/products") {
        ...
        setNotice("Produto incluído no catálogo.");
      } else {
  ...
  async function submitRegulatoryReview(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const authHeaders = await getAuthHeaders();
      const response = await fetch(`/api/admin/products/${encodeURIComponent(productId)}/regulatory`, {
        method: "PATCH",
        headers: {
          ...authHeaders,
          "content-type": "application/json",
        },
        body: JSON.stringify({ decision: data.get("decision"), requiresPrescription: data.get("requiresPrescription") === "on" }),
      });
      const body = await response.json() as { error?: string };
    ...
    <div className="dashboard-shell">
      <aside className="dashboard-sidebar">
        ...
        <div className="sidebar-user">
          ...
          <p><strong>{userName}</strong><small>{demo ? "ambiente demonstrativo" : userEmail}</small></p>
          <button onClick={handleSignOut} title="Sair da conta" style={{ background: "transparent", border: "none", color: "inherit", cursor: "pointer", fontSize: "1.2rem", padding: "4px" }}>
            🚪
          </button>
        </div>
      </aside>
      ...
      <main className="dashboard-main">
        ...
        {view === "catalog" && <CatalogView formOpen={formOpen} setFormOpen={setFormOpen} onSubmit={submitDemoOrApi} onDeleteProduct={handleDeleteProduct} products={adminProducts} query={catalogQuery} setQuery={setCatalogQuery} loading={catalogLoading} error={catalogError} />}
...
function CatalogView({ formOpen, setFormOpen, onSubmit, onDeleteProduct, products, query, setQuery, loading, error }: CatalogViewProps) {
      ...
      <section className="dashboard-card full-card">
        ...
        {formOpen === "product" && (
          <form className="admin-form" onSubmit={(event) => onSubmit(event, "/api/admin/products")}>
            ...
            <div className="form-actions wide"><button type="button" onClick={() => setFormOpen(null)}>Cancelar</button><button type="submit">Cadastrar produto</button></div>
          </form>
        )}

        <div className="admin-catalog-summary"><strong>{visible.length}</strong><span>{visible.length === 1 ? "produto exibido" : "produtos exibidos"}</span></div>
              ...
              <article className="admin-product-card" key={product.id}>
                <header>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <StatusPill>{productState(product)}</StatusPill>
                    {onDeleteProduct && (
                      <button
                        type="button"
                        onClick={() => onDeleteProduct(product.id)}
                        title="Remover produto"
                        style={{ background: "transparent", border: "none", color: "var(--coral, #d9534f)", cursor: "pointer", fontSize: "0.85rem", padding: "0 4px" }}
                      >
                        ✕
                      </button>
                    )}
...
function MarketingView({ formOpen, setFormOpen, onSubmit }: FormProps) {
    ...
    async function loadMarketing() {
      try {
        ...
        if (bRes.ok) {
          ...
          if (bData.banners?.length) setBanners(bData.banners);
        }
        if (dRes.ok) {
          const dData = await dRes.json();
          if (dData.discounts?.length) setDiscounts(dData.discounts);
        }
      } catch {
        // use default display
      }
    ...
    <div className="dashboard-stack">
      <div className="dashboard-columns marketing-columns">
        <section className="dashboard-card">
          <CardHeading title="Banners" action="Novo banner" onClick={() => setFormOpen(formOpen === "banner" ? null : "banner")} />
          {formOpen === "banner" && (
            <form className="admin-form single" onSubmit={(event) => onSubmit(event, "/api/admin/banners")}>
              <label>Título<input name="title" required maxLength={100} /></label>
              <label>Subtítulo<input name="subtitle" required maxLength={180} /></label>
              <label>Texto do botão<input name="ctaLabel" required maxLength={40} /></label>
              <label>Destino<input name="ctaHref" defaultValue="/catalogo" required /></label>
              <div className="form-actions"><button type="button" onClick={() => setFormOpen(null)}>Cancelar</button><button type="submit">Publicar banner</button></div>
            </form>
          )}
          {banners.length > 0 ? (
            banners.map((b) => (
              <div className="campaign-card live" key={b.id}>
                <span>{b.is_active ? "Ativo" : "Inativo"}</span>
                <h3>{b.title}</h3>
                <p>{b.subtitle}</p>
                <small>{b.cta_href}</small>
              </div>
            ))
          ) : (
            <>
              <div className="campaign-card live"><span>Ativo</span><h3>Cuidar do dia a dia pode ser mais simples.</h3><p>Banner principal • Página inicial</p><small>Sem data de término</small></div>
              <div className="campaign-card"><span>Agendado</span><h3>Semana do autocuidado</h3><p>Banner secundário • Dermocosméticos</p><small>Inicia em breve</small></div>
            </>
          )}
        </section>

        <section className="dashboard-card">
          <CardHeading title="Cupons e descontos" action="Novo desconto" onClick={() => setFormOpen(formOpen === "discount" ? null : "discount")} />
          {formOpen === "discount" && (
            <form className="admin-form single" onSubmit={(event) => onSubmit(event, "/api/admin/discounts")}>
              <label>Nome<input name="name" required maxLength={80} /></label>
              <label>Código<input name="code" required maxLength={24} /></label>
              <label>Tipo<select name="kind"><option value="percent">Percentual (%)</option><option value="fixed">Valor fixo (centavos)</option></select></label>
              <label>Valor<input name="amount" type="number" min="1" required /></label>
              <label>Mínimo em centavos<input name="minSubtotalCents" type="number" min="0" defaultValue="0" /></label>
              <div className="form-actions"><button type="button" onClick={() => setFormOpen(null)}>Cancelar</button><button type="submit">Ativar desconto</button></div>
            </form>
          )}
          {discounts.length > 0 ? (
            discounts.map((d) => (
              <div className="coupon-card" key={d.id}>
                <span>{d.code}</span>
                <strong>{d.kind === "percent" ? `${d.amount}% OFF` : `R$ ${(d.amount / 100).toFixed(2)} OFF`}</strong>
                <p>{d.name} • {d.min_subtotal_cents ? `mínimo de R$ ${(d.min_subtotal_cents / 100).toFixed(2)}` : "sem valor mínimo"}</p>
              </div>
            ))
          ) : (
            <>
              <div className="coupon-card"><span>BEMVINDO10</span><strong>R$ 10 OFF</strong><p>Primeiro pedido • mínimo de R$ 50</p></div>
              <div className="coupon-card muted"><span>DERMA20</span><strong>20% OFF</strong><p>Categoria dermocosméticos</p></div>
            </>
          )}
        </section>
      </div>
...
function TeamView({ formOpen, setFormOpen, onSubmit }: FormProps) {
  const [team, setTeam] = useState<Array<{ email: string; role: string; updated_at?: string }>>([]);
  ...
  async function loadTeam() {
    ...
    try {
      const authHeaders = await getAuthHeaders();
      const response = await fetch("/api/admin/users/roles", { headers: authHeaders });
      if (response.ok) {
  ...
  const matrix = [
    { role: "Proprietário (Owner)", permissions: "Acesso total: gerencia equipe, define permissões, pedidos, finanças e auditoria" },
    { role: "Gerente", permissions: "Operação geral, catálogo, campanhas de marketing e gestão de pedidos" },
    { role: "Farmacêutico", permissions: "Revisão e aprovação de receitas médicas, classificação regulatória e dispensação" },
    { role: "Catálogo", permissions: "Cadastro de produtos, controle de estoque, banners e cupons" },
    { role: "Atendimento", permissions: "Consulta de pedidos, suporte ao cliente e cancelamentos autorizados" },
  ];

  return (
    <div className="dashboard-stack">
      <section className="dashboard-card full-card">
        <CardHeading title="Equipe e permissões do sistema" action="Delegar / Conceder acesso" onClick={() => setFormOpen(formOpen === "role" ? null : "role")} />
        
        {formOpen === "role" && (
          <form
            className="admin-form role-form"
            onSubmit={async (event) => {
              await onSubmit(event, "/api/admin/users/roles");
              loadTeam();
            }}
          >
            <label>E-mail do usuário<input name="email" type="email" placeholder="usuario@email.com" required /></label>
            <label>Função / Perfil delegado<select name="role">
            ...
            <div className="form-actions"><button type="button" onClick={() => setFormOpen(null)}>Cancelar</button><button type="submit">Salvar permissão</button></div>
          </form>
        )}

        <div style={{ marginTop: "16px", marginBottom: "20px" }}>
          <h3 style={{ fontSize: "1rem", margin: "0 0 10px", color: "var(--foreground)" }}>Membros cadastrados no Supabase</h3>
          {teamLoading && <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>Carregando membros da equipe…</p>}
          {!teamLoading && team.length === 0 && (
            <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>Nenhum outro membro cadastrado ainda. Use o botão acima para delegar acessos.</p>
          )}
          {!teamLoading && team.length > 0 && (
                ...
                <article key={m.email} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <div>
                      ...
                      <small style={{ display: "block" }}>Perfil: {roleLabels[m.role as DashboardRole] || m.role}</small>
                    </div>
                  </div>
                  <span className={`status-pill ${m.role === "owner" ? "success" : "info"}`}>
                    {roleLabels[m.role as DashboardRole] || m.role}
                  </span>
                </article>
              ))}
            </div>
        ...
        <h3 style={{ fontSize: "1rem", margin: "24px 0 10px", color: "var(--foreground)" }}>Hierarquia de perfis disponíveis</h3>
        <div className="permission-list">
          {matrix.map((item) => (
            <article key={item.role}>
              <span><Icon name="shield" size={18} /></span>
              <p><strong>{item.role}</strong><small>{item.permissions}</small></p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="dashboard-card audit-preview">
        <CardHeading title="Auditoria e segurança" />
        <p><strong>user.role.assign</strong> - Atribuição de permissões pelo Proprietário <span>Segurança ativa</span></p>
        <p><strong>catalog.product.create</strong> - Gestão de catálogo auditada <span>Supabase logs</span></p>
        <p><strong>prescription.review</strong> - Análise de receitas restrita a farmacêuticos <span>Conformidade ANVISA</span></p>
      </section>
    </div>
  );
}
